...
REFF_URL = os.getenv('REFF_URL', '')  # Tambahkan URL referal

BASE_URL = 'https://api.simplesvg.com'
...
# ===== Fitur Auto Reff =====
async def auto_reff(task_data):
    """Mengirimkan referal secara otomatis."""
    if not REFF_URL:
        print(Fore.RED + "❌ REFF_URL tidak ditemukan dalam konfigurasi .env" + Style.RESET_ALL)
        return

    payload = {
        'task': task_data,
        'reff_url': REFF_URL  # Sertakan URL referal dalam payload
    }
    response = await call_api('/reff/auto', method='POST', payload=payload)
    if response:
        print(f"✅ Auto Reff berhasil: {response}")
    else:
        print("❌ Gagal menjalankan Auto Reff.")

# ===== Fitur Informasi Akun =====
async def fetch_account_info(client):
    """Mengambil informasi akun dari API."""
    try:
        me = await client.get_me()
        print(Fore.MAGENTA + f"Akun: {me.first_name} {me.last_name or ''}, Username: {me.username}" + Style.RESET_ALL)
    except Exception as e:
        print(Fore.RED + f"❌ Gagal mengambil informasi akun: {e}" + Style.RESET_ALL)
...
async def contribute_engine_data():
    """Berkontribusi data ke API."""
    payload = {'from_date': '2024-12-16', 'quality_connection': 75}
...
async def run_session(session_name, index, proxies):
    ...
    await fetch_account_info(client)

    # Jalankan tugas auto_reff jika diaktifkan
    if AUTO_TASK:
        await auto_reff({'session': session_name})
